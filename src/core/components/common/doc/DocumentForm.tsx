...
import { DocumentTypeConfig } from './types/document';
...
const DocumentForm: React.FC<DocumentFormProps> = ({ documentType, onSubmit, initialData }) => {
  ...
  useEffect(() => {
    const loadSchema = async () => {
      try {
        ...
        const fullImportPath = `../schemas/${documentType.type_id}.json`;
        
        const schemaLoader = schemaModules[fullImportPath];
        if (!schemaLoader) {
          // fallback to generic
          console.warn(`Schema not found: ${fullImportPath}`);
          setLoading(false);
          return;
        }
  ...
  const renderFormFields = () => {
    ...
    Object.entries(properties).forEach(([key, property]: [string, any]) => {
      if (key === 'items' || key === 'signature') return; 
  ...
  return (
    <Card>
      <Title level={3}>Create {documentType.display_name}</Title>
      <Form
        form={form}
        layout="vertical"
