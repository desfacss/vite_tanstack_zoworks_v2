...
        const localViewConfig = await import(`../../schemas/viewConfig/${entityType}.json`);
        return { config: localConfig.default, viewConfig: localViewConfig.default };
      } catch (error) {
        console.warn(`Local config file for ${entityType} not found, falling back to database.`, error);
      }
...
      return { config: mergedConfig, viewConfig: viewConfigData };
    },
