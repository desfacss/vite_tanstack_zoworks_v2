// src/modules/shop/pages/CategoryLandingPage.tsx
...
import { getProducts, getCategoryBySlug } from '../services/dataService';
import type { Product, Category, ProductFilters } from '../types';

// Using a similar card structure as PLP for consistency
...
const CategoryLandingPage: React.FC = () => {
  ...
  async function loadCategory() {
    setLoading(true);
    const cat = await getCategoryBySlug(orgId, slug!);
    setCategory(cat);
    if (cat) {
  ...
  if (!category) return <div className="shop-container shop-empty"><h3>Category Not Found</h3><Link to="/shop">Back to Shop</Link></div>;

  return (
    <div className="shop-container shop-page shop-fade-in">
      <div className="shop-breadcrumb">
        <Link to="/shop">Home</Link> <ChevronRight size={12} /> <span>{category.name}</span>
      </div>

      <header className="shop-category-header" style={{ marginBottom: 32 }}>
        <h1 className="shop-page-title">{category.name}</h1>
        {category.description && <p className="shop-category-description" style={{ color: 'var(--shop-muted)', marginTop: 8, maxWidth: 800 }}>{category.description}</p>}
      </header>

      <div className="shop-toolbar" style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span className="shop-results-count">{total} products in this category</span>
        </div>
