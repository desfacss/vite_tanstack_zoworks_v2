...
interface TimelineViewConfigData {
  fields?: {
    name?: string;
    start_date?: string;
    end_date?: string;
...
interface TimelineViewConfigProps {
  configData: TimelineViewConfigData;
  onSave: (data: TimelineViewConfigData) => void;
  availableColumns: any[];
  metadata?: MetadataItem[];
}

const TimelineViewConfig: React.FC<TimelineViewConfigProps> = ({
  configData,
  onSave,
  availableColumns,
  metadata,
  ...
  const [fields, setFields] = useState<Field[]>(() => {
    const initialFields: Field[] = [
      ...
      { order: 3, fieldName: configData?.fields?.end_date || '', fieldPath: configData?.fields?.end_date || '' },
      { order: 4, fieldName: configData?.fields?.group_by_field || '', fieldPath: configData?.fields?.group_by_field || '' },
    ];
    // Load additional fields from configData
    if (configData?.fields) {
      Object.entries(configData.fields).forEach(([key, value]) => {
        if (!['name', 'start_date', 'end_date', 'group_by_field'].includes(key) && value) {
          initialFields.push({
  ...
  const handleRemoveField = (index: number) => {
    if (fields.length > 4) { // Ensure at least 4 fields remain
      ...
      message.warning('Cannot remove required fields (name, start_date, end_date, group_by_field).');
    }
  };
  ...
  const handleSaveConfig = () => {
    const updatedConfig: TimelineViewConfigData = {
      fields: {
        name: fields.find(f => f.order === 1)?.fieldPath || '',
        start_date: fields.find(f => f.order === 2)?.fieldPath || '',
        end_date: fields.find(f => f.order === 3)?.fieldPath || '',
        group_by_field: fields.find(f => f.order === 4)?.fieldPath || '',
      },
      actions,
      exportOptions,
      showFeatures,
    ...
    message.success('Timeline configuration saved successfully!');
  };
  ...
  const fieldColumns = [
    ...
    },
    {
      ...
      key: 'fieldRole',
      render: (order: number) => {
        const roles = ['name', 'start_date', 'end_date', 'group_by_field'];
  ...
  return (
    <div>
      <h2>Timeline View Configuration</h2>
...
export default TimelineViewConfig;
