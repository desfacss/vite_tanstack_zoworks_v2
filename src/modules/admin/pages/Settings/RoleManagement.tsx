...
const RoleManagement: React.FC<RoleManagementProps> = ({ organization, user }) => {
    ...
    const fetchAllRoles = async () => {
      try {
        ...
        const { data: rolesData, error: rolesError } = await supabase
          .schema('identity').from('roles')
          .select('id, name, permissions, is_sassadmin, is_active, organization_id, feature')
          .eq('is_active', true)
          .order('name', { ascending: true });
        ...
        const groupedRoles = (rolesData || []).reduce((acc: { [key: string]: Role[] }, role: any) => {
        ...
        const grouped = Object.entries(groupedRoles).map(([orgId, roles]) => ({
          organization: orgId === 'no-org' ? null : (orgsData || []).find((org: any) => org.id === orgId) || null,
          roles: roles as Role[],
        }));
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const newRoles = (selectedRoles || []).map((role: any) => ({
        name: role.name,
