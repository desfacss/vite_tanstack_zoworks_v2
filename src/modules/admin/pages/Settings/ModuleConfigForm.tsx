  ...
  useEffect(() => {
    const fetchModuleData = async () => {
      ...
      try {
        ...
        const { data: configData, error: configError } = await supabase.schema('identity').rpc('get_organization_module_configs', {
        ...
        Object.entries(configData || {}).forEach(([moduleName, configModule]) => {
          mergedHierarchy[moduleName] = {
            sub_modules: (configModule as Module).sub_modules || {},
            settings: (configModule as Module).settings || mergedHierarchy[moduleName]?.settings || {},
          };
        });
  ...
  const handleSubModuleChange = (
    ...
    setModuleHierarchy((prev) => {
      ...
      if (!updated[moduleName].sub_modules) {
        // We only delete the module completely if they also didn't have it checked, but this UX requires us to keep the module if it's checked.
        // For simplicity, we keep the module empty. 
        // Or actually, let's keep it here because handleModuleChange handles module deletion.
      }
      return updated;
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Object.entries(moduleHierarchy).forEach(([moduleName, moduleData]) => {
        ...
        if (Object.keys(sub_modules).length > 0 || moduleData.settings) {
      ...
      setSaving(true);
      const { error } = await supabase.schema('identity').rpc('save_module_configs', payload);
      ...
      <Panel
        header={
          <Checkbox
            ...
            onClick={(e) => e.stopPropagation()} // Prevent toggling accordion when clicking the checkbox directly
          >
            {moduleName}
          </Checkbox>
        }
