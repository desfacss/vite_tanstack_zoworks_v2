...
interface Location {
  ...
  app_settings: { // 💡 Added app_settings to Location interface
    support?: {
      email?: {
        email?: string
        fromName?: string
...
const LocationSettings: React.FC = () => {
  ...
  const fetchLocations = async () => {
    const { data, error } = await supabase
      .schema('identity')
      .from('locations')
      .select('*, app_settings') // 💡 Select app_settings
  ...
  const handleEdit = (record: Location) => {
    ...
    const initialValues = {
      ...
      app_settings: record?.app_settings || {}, // 💡 Set initial value for app_settings
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editItem) {
        ...
        // ... (insert logic remains the same as it's a new entry)
        const { error } = await supabase
        .schema('identity')
        .from('locations')
        .insert(payload)
